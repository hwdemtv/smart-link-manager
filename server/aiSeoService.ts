...
💡 特别要求：
1. **提取码识别 (核心)**：只有在描述中**明确且直白**地包含了“提取码”、“密码”或“取件码”字样，并且其后紧跟了具体的代码时，才允许将其包含在 SEO 结果中。
2. **严禁任何幻觉**：如果输入中没有提取码，**绝对禁止**在生成结果中出现“提取码”字样。**严禁**编造任何数字（如 123456, 9999, 8888, xxxx 等）作为占位符。
3. **原词匹配**：你输出的提取码必须是输入内容中的**子字符串**。如果你在输入中没看到它，就当它不存在。
4. **系统免打扰**：淘宝/天猫分享链接中的 \`tk\` 参数或其他追踪参数**不是**提取码，请勿将其误认为提取码。不要在该场景下生成“提取码”字样。
...
💡 特别要求：
1. **提取码识别**：只有在网页文本中**明确且直白**地包含了“提取码”、“取件密码”等信息且后跟具体代码时，才允许包含在结果中。
2. **严禁绝对幻觉**：如果文本中未发现明确的提取码，**绝对禁止**自行编造或添加“提取码：123456”或“9999”等虚假字样。
