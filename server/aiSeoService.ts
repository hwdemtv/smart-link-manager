...
seoTitle 建议 20-40 个汉字，seoDescription 建议 80-150 个汉字。
 ...
 💡 负向约束 (必须遵守)：
 1. **严禁幻觉识别**：只有当描述中**显式**出现“提取码：xxxx”或“密码：xxxx”且代码真实存在时才可保留。**严禁**识别 URL 中的参数（如 tk, code, id 等）。
 2. **淘宝/天猫特别策略**：该类链接**绝对没有**提取码。严禁在生成结果中出现“提取码”三个字，严禁生成任何占位符（如 XXX, 1234, 见详情等）。
 3. **原词检查**：生成的提取码必须是输入内容的一个**子字符串**。如果你没看到代码，就当它不存在。
 4. **字段纯净度**：若无提取码，seoTitle 和 seoDescription 中**严禁**包含“提取码”或“密码”字样。

重要：只返回一个 JSON 对象，严禁返回 Markdown 代码块，严禁在 JSON 外添加任何文字：
...
seoTitle 建议 40-60 个汉字，seoDescription 建议 100-160 个汉字。
 ...
 💡 负向约束 (必须遵守)：
 1. **严禁幻觉识别**：只有当网页内容描述中**显式**出现“提取码：xxxx”或“取件码：xxxx”且代码真实存在时才可保留。**严禁**误认 URL 参数（如 tk, pwd, sid 等）为提取码。
 2. **淘宝/天猫特别策略**：这类电商链接**绝对不含**提取码。禁止在生成结果中出现“提取码”、“密码”字句，严禁生成任何类似“XXX”、“见详情”、“1234”的伪代码。
 3. **原词匹配**：提取码必须是描述内容中的**原词子字符串**。
 4. **字段纯净度**：如果描述中没写提取码，生成的 seoTitle 和 seoDescription 中**严禁**出现“提取码”或“密码”等引导词。
