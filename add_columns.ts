...
async function main() {
  ...
  try {
    await connection.query("ALTER TABLE `users` ADD COLUMN `isActive` INT NOT NULL DEFAULT 1;");
  ...
  try {
    await connection.query("ALTER TABLE `users` ADD COLUMN `lastIpAddress` VARCHAR(45);");
