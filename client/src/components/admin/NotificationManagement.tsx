...
import { Bell, Send, Trash2, Check, AlertTriangle, Info, Megaphone, Users, Eye } from "lucide-react";
...
export default function NotificationManagement() {
  ...
  // Queries
  const { data: notifications, isLoading, refetch } = trpc.user.listNotifications.useQuery({
    limit: itemsPerPage,
  ...
  const sendMutation = trpc.user.sendNotification.useMutation({
    onSuccess: (result) => {
      const countText = result.count === -1 ? t("admin.notify.allUsers") : `${result.count} ${t("admin.notify.allUsers").includes("All") ? "users" : "位用户"}`;
      toast.success(`${t("admin.notify.sendSuccess")} ${countText}`);
      setIsSendDialogOpen(false);
      setSendForm({ title: "", message: "", type: "announcement", priority: "normal" });
      refetch();
    },
    onError: (error: any) => {
  ...
  const getTypeBadge = (type: string) => {
    const colors: Record<string, string> = {
      ...
      link_invalid: "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400",
      link_expired: "bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400",
    };
    return (
      ...
      <Card>
          ...
          <div className="flex items-center gap-6 mt-4 pt-4 border-t">
            {stats && (
              <>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">{t("admin.notify.total")}:</span>
                  <span className="font-semibold">{stats.total}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Eye className="w-4 h-4 text-muted-foreground" />
                  <span className="text-sm text-muted-foreground">{t("admin.notify.unread")}:</span>
            ...
            <div className="ml-auto">
              <Select value={typeFilter} onValueChange={setTypeFilter}>
                ...
                <SelectContent>
                  <SelectItem value="all">{t("admin.notify.allTypes")}</SelectItem>
                  <SelectItem value="announcement">{t("admin.notify.typeAnnouncement")}</SelectItem>
                  <SelectItem value="warning">{t("admin.notify.typeWarning")}</SelectItem>
                  <SelectItem value="info">{t("admin.notify.typeInfo")}</SelectItem>
                  <SelectItem value="system">{t("admin.notify.typeSystem")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">{t("admin.notify.noNotifications")}</div>
          ) : (
            <>
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">{t("admin.notify.actions")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                      ...
                      <TableCell>
                        {notification.userId ? (
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-xs">User #{notification.userId}</span>
                          </div>
                        ) : (
              ...
              <div className="flex items-center justify-between px-2 py-4 border-t border-border">
                <div className="text-sm text-muted-foreground">
                  {t("admin.pagination.showing", {
                    ...
                    to: Math.min(currentPage * itemsPerPage, (notifications as NotificationType[]).length),
      ...
      <Dialog open={isSendDialogOpen} onOpenChange={setIsSendDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="notify-message">{t("admin.notify.message")} *</Label>
              <Textarea
                id="notify-message"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="notify-priority">{t("admin.notify.priority")}</Label>
                <Select
                  value={sendForm.priority}
                  onValueChange={(value: any) => setSendForm({ ...sendForm, priority: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="low">{t("admin.notify.priorityLow")}</SelectItem>
                    <SelectItem value="normal">{t("admin.notify.priorityNormal")}</SelectItem>
                    <SelectItem value="high">{t("admin.notify.priorityHigh")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSendNotification} disabled={sendMutation.isPending}>
              {sendMutation.isPending ? t("admin.notify.sending") : t("admin.notify.send")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
