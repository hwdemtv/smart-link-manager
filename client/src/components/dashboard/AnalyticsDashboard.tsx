    ...
    <g>
      ...
      {width > 60 && height > 40 && (
        <>
          ...
          <text x={x + width / 2} y={y + height / 2 + 12} textAnchor="middle" fill="rgba(255,255,255,0.8)" fontSize={width > 100 ? 12 : 10}>
            {value} 次点击
          </text>
        </>
      )}
...
export function AnalyticsDashboard() {
  ...
  // 拉取过去 7 天的全局大盘数据
  const { data: stats, isLoading } = trpc.links.globalStats.useQuery({ days: 7 });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
      ...
      {cityData.length > 0 && (
        <Card className="p-4">
          ...
          <div className="h-[220px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={cityData} layout="vertical" margin={{ left: 0, right: 16 }}>
      ...
      <Card className="p-4 lg:col-span-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* 浏览器 */}
          <div>
            <h4 className="text-sm font-medium text-muted-foreground mb-3">浏览器</h4>
          ...
          {/* 国家列表 */}
          <div>
            <h4 className="text-sm font-medium text-muted-foreground mb-3">国家 / 地区</h4>
            <div className="space-y-2">
              {countryData.slice(0, 5).map((item, i) => {
                ...
                return (
                  <div key={item.name} className="flex items-center gap-3">
                    <span className="text-sm w-20 truncate">{COUNTRY_FLAGS[item.name] || '🌐'} {item.name}</span>
                    <div className="flex-1 bg-muted rounded-full h-2.5 overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all duration-500"
                        style={{ width: `${pct}%`, backgroundColor: TREEMAP_COLORS[i % TREEMAP_COLORS.length] }}
                      />
                    </div>
                    <span className="text-xs text-muted-foreground w-12 text-right">{item.value}</span>
                  </div>
