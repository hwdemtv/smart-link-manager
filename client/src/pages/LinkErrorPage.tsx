  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans selection:bg-red-100">
      <Card className="w-full max-w-lg overflow-hidden border-none shadow-2xl relative">
        ...
        <div className="p-10 space-y-8 text-center">
          ...
          <div className="relative inline-block">
            <div className={`absolute inset-0 blur-2xl opacity-20 rounded-full ${isExpired ? 'bg-amber-500' : 'bg-red-500'}`} />
            <div className={`relative w-24 h-24 rounded-3xl flex items-center justify-center mx-auto ${isExpired ? 'bg-amber-50 text-amber-600' : 'bg-red-50 text-red-600'}`}>
              {isExpired ? (
      ...
      <div className="fixed top-0 left-0 w-full h-full -z-10 overflow-hidden pointer-events-none">
        <div className={`absolute top-[-20%] left-[-10%] w-[60%] h-[60%] rounded-full blur-[150px] opacity-20 ${isExpired ? 'bg-amber-200' : 'bg-red-200'}`} />
