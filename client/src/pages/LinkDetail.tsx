...
export default function LinkDetail() {
  ...
  const copyToClipboard = async () => {
    ...
    if (success) {
      ...
      toast.error(t("linkDetail.copyFailed", "复制失败，请手动复制"));
    }
  };
  ...
  const formatDataForPie = (
    dataRecord: Record<string, number> | undefined,
    limit = 5
  ) => {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container py-8">
          ...
          <Card className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">
                  {link.customDomain || t("linkDetail.default")}
                </p>
              </div>
            </div>
          </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-8">
          <StatPieChart
            title={t("linkDetail.deviceDistribution")}
            data={formatDataForPie(stats?.deviceStats)}
          />
          <StatPieChart
            title={t("common.browser", "Browser")}
            data={formatDataForPie(stats?.browserStats)}
          />
          <StatPieChart
            title={t("common.os", "Operating System")}
            data={formatDataForPie(stats?.osStats)}
          />
          <StatPieChart
            title={t("common.country", "Country / Region")}
            data={formatDataForPie(stats?.countryStats)}
          />

          {link?.abTestEnabled === 1 && (
            <StatPieChart
              title={t(
                "linkDetail.variantDistribution",
                "A/B 变体流向 (Variant Traffic)"
              )}
              data={formatDataForPie((stats as any)?.variantStats)}
            />
          )}
        </div>
        ...
        {link?.abTestEnabled === 1 && link?.abTestUrl && (
          <Card className="p-6 mt-4">
            <h2 className="text-xl font-semibold mb-4">
              {t("linkDetail.variantBUrl", "变体 B 目标链接 (Variant B URL)")}
            </h2>
            <a
              href={link.abTestUrl}
              target="_blank"
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="text-left py-2 px-4">
                      {t("linkDetail.browser")}
                    </th>
                    <th className="text-left py-2 px-4">
                      {t("linkDetail.os")}
                    </th>
                    <th className="text-left py-2 px-4">
                      {t("linkDetail.country")}
                    </th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    .map((click: any, index: number) => (
                      <tr key={index} className="border-b border-border">
                        ...
                        <td className="py-2 px-4 capitalize">
                          {click.deviceType || t("linkDetail.unknown")}
                        </td>
                        <td className="py-2 px-4">
                          {click.browserName || t("linkDetail.unknown")}
                        </td>
                        <td className="py-2 px-4">
                          {click.osName || t("linkDetail.unknown")}
                        </td>
                        <td className="py-2 px-4">
                          {click.country || t("linkDetail.unknown")}
                        </td>
                      </tr>
                    ))}
                </tbody>
